  ...
  document.addEventListener('visibilitychange', async () => {
    try {
      if (document.visibilityState === 'hidden') {
        ...
        // [YENİ] Anlık fotoğraf — personel ekranı kapattığında oturum verisini
        // localStorage'a yaz. Geri döndüğünde flash gösterilecek.
        // logSessionResult'a bağımlı değil; sepet ya da blur varsa hep yakalar.
        if (!isAdmin() && currentUser) {
          const _bc = Object.keys(_sessionData.blurUrunler || {}).length;
          if (basket.length > 0 || _bc > 0) {
            const _snap = {
              sonuc:     'belirsiz',           // modal henüz açılmadı
              neden:     '',
              urunSayisi: basket.length,
              blurSayisi: _bc,
              sure:      _sessionData.startTime
                           ? Math.round((Date.now() - _sessionData.startTime) / 60000)
                           : 0,
              tutar:     basket.reduce((s, i) => s + (i.nakit - (i.itemDisc || 0)), 0),
              bakildi:   (_sessionData.revealedPrices || []).slice(),
              sepete:    basket.map(i => i.urun),
              ts:        Date.now()
            };
            localStorage.setItem('_pendingFlash', JSON.stringify(_snap));
          }
        }

      } else if (document.visibilityState === 'visible' && currentUser && _db) {

        // [YENİ] Dönüşte bekleyen fotoğraf varsa göster (modal'a basmadan çıktıysa)
        // Sadece sepet hâlâ doluysa değil — sepet kapanmışsa da göster
        try {
          const _pf = localStorage.getItem('_pendingFlash');
          if (_pf) {
            const _pfData = JSON.parse(_pf);
            const _pfYas = (Date.now() - (_pfData.ts || 0)) / 1000;
            // 30 saniyeden eski değilse göster (henüz modal kapatılmamış olabilir)
            // 300 saniyeden eskiyse gösterme (oturum çok eski)
            if (_pfYas > 2 && _pfYas < 300 && basket.length === 0) {
              localStorage.removeItem('_pendingFlash');
              _showSessionFlash(_pfData);
            } else if (_pfYas >= 300 || basket.length > 0) {
              // Oturum devam ediyor ya da çok eski — sil
              localStorage.removeItem('_pendingFlash');
            }
          }
        } catch(e) { localStorage.removeItem('_pendingFlash'); }
...
let _flashOzetData = null;       // [YENİ] Oturum kapanınca micro-flash'a taşınan özet
...
window.clearBasket = function(bypass = false, sonucOverride = null, nedenOverride = '') {
  ...
  const handleSonuc = async (sonuc, neden = '') => {
    modal.style.display = 'none';
    if (kpanel) kpanel.style.display = 'none';
    // Pending snapshot'ı temizle — asıl flash logSessionResult→_doClearBasket'ten gelecek
    localStorage.removeItem('_pendingFlash');
    try {
...
// ─── OTURUM KAPANIŞ MICRO-FLASH ────────────────────────────────
// Müşteri oturumu kapandıktan 400ms sonra 5 saniyelik özet gösterir.
// Performans butonu kaldırılmamıştır — bu ek, isteğe bağlı kıyası değiştirmez.
function _showSessionFlash(ozet) {
  if (!ozet) return;
  // Anlamlı oturum değilse (hiç blur yok, hiç ürün yok) gösterme
  if (ozet.blurSayisi === 0 && ozet.urunSayisi === 0) return;

  setTimeout(() => {
    // Varsa eski flash'ı temizle
    document.getElementById('_session-flash')?.remove();

    // Renk ve ikon — sonuca göre
    const renkMap = {
      satis:   { bg: '#f0fdf4', border: '#16a34a', text: '#15803d', icon: '✅' },
      kacti:   { bg: '#fefce8', border: '#ca8a04', text: '#92400e', icon: '⚡' },
      default: { bg: '#f8fafc', border: '#64748b', text: '#334155', icon: '📋' }
    };
    const r = renkMap[ozet.sonuc] || renkMap.default;

    // Süresi — 0 dk ise gösterme
    const sureTxt = ozet.sure > 0 ? `${ozet.sure} dk` : '';

    // Tutarı formatla
    const tutarTxt = ozet.tutar > 0
      ? new Intl.NumberFormat('tr-TR', { style:'currency', currency:'TRY', maximumFractionDigits:0 }).format(ozet.tutar)
      : '';

    // Bakılan ama alınmayan ürünler
    const alinmayan = ozet.bakildi.filter(u => !ozet.sepete.includes(u));

    // HTML
    const flash = document.createElement('div');
    flash.id = '_session-flash';
    flash.style.cssText = [
      'position:fixed', 'bottom:72px', 'left:50%', 'transform:translateX(-50%)',
      'z-index:9999', 'width:calc(100% - 32px)', 'max-width:420px',
      `background:${r.bg}`, `border:1.5px solid ${r.border}`, 'border-radius:12px',
      'padding:12px 14px', 'box-shadow:0 4px 18px rgba(0,0,0,.13)',
      'font-size:.72rem', 'line-height:1.5', 'animation:_flashIn .25s ease'
    ].join(';');

    flash.innerHTML = `
      <style>
        @keyframes _flashIn { from { opacity:0; transform:translateX(-50%) translateY(12px); } to { opacity:1; transform:translateX(-50%) translateY(0); } }
        #_session-flash ._sf-row { display:flex; align-items:center; gap:8px; }
        #_session-flash ._sf-chip { background:rgba(0,0,0,.07); border-radius:6px; padding:2px 7px; font-size:.65rem; font-weight:600; white-space:nowrap; }
        #_session-flash ._sf-miss { color:#dc2626; font-size:.64rem; margin-top:5px; }
        #_session-flash ._sf-close { position:absolute; top:8px; right:10px; cursor:pointer; font-size:.80rem; color:#94a3b8; padding:2px 5px; }
        #_session-flash ._sf-bar { height:3px; background:${r.border}; border-radius:2px; margin-top:9px; animation:_sfBar 5s linear forwards; }
        @keyframes _sfBar { from { width:100%; } to { width:0%; } }
      </style>
      <span class="_sf-close" onclick="document.getElementById('_session-flash')?.remove()">✕</span>
      <div class="_sf-row">
        <span style="font-size:1.1rem">${r.icon}</span>
        <span style="font-weight:700;color:${r.text};font-size:.75rem">Oturum özeti</span>
        ${sureTxt ? `<span class="_sf-chip">${sureTxt}</span>` : ''}
        ${tutarTxt ? `<span class="_sf-chip" style="color:${r.text}">${tutarTxt}</span>` : ''}
      </div>
      <div style="display:flex;gap:5px;flex-wrap:wrap;margin-top:6px">
        ${ozet.urunSayisi > 0 ? `<span class="_sf-chip">🛒 ${ozet.urunSayisi} ürün</span>` : ''}
        ${ozet.blurSayisi > 0 ? `<span class="_sf-chip">👁 ${ozet.blurSayisi} fiyat sorgusu</span>` : ''}
        ${ozet.neden ? `<span class="_sf-chip">↩ ${ozet.neden}</span>` : ''}
      </div>
      ${alinmayan.length > 0
        ? `<div class="_sf-miss">⚠ Fiyat sorulup alınmayan: ${alinmayan.slice(0, 3).join(', ')}${alinmayan.length > 3 ? ` +${alinmayan.length - 3}` : ''}</div>`
        : ''}
      <div class="_sf-bar"></div>
    `;
...
function _doClearBasket() {
  ...
  updateCartUI();
  // [YENİ] Oturum kapandıktan sonra micro-flash özet göster
  _showSessionFlash(_flashOzetData);
  _flashOzetData = null;
}
...
async function _feedbackSelect(sonuc) {
  ...
  _floatingBarActive = false;
  document.getElementById('_float-feedback')?.remove();
  // Pending snapshot'ı temizle — flash _doClearBasket'ten gelecek
  localStorage.removeItem('_pendingFlash');
...
async function logSessionResult(sonuc, neden) {
  ...
  // [YENİ] Micro-flash için oturum özetini _doClearBasket çağrılmadan önce yakala
  // Admin'de gösterme; anlamlı oturum varsa (sepet > 0 veya blur > 0) hazırla
  if (!isAdmin() && (basket.length > 0 || _blurCount > 0)) {
    const _ozSure  = _sessionData.startTime ? Math.round((Date.now() - _sessionData.startTime) / 60000) : 0;
    const _ozTutar = basket.reduce((s, i) => s + (i.nakit - (i.itemDisc || 0)), 0);
    _flashOzetData = {
      sonuc,
      neden:      neden || '',
      urunSayisi: basket.length,
      blurSayisi: _blurCount,
      sure:       _ozSure,
      tutar:      _ozTutar,
      bakildi:    (_sessionData.revealedPrices || []).slice(),
      sepete:     basket.map(i => i.urun)
