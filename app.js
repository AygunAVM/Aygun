// ═══════════════════════════════════════════════════════════════
//  AYGÜN AVM — app.js  (Rev 7.4 — Okuma Kotası Optimizasyonu)
...
function startFirebaseListeners() {
  ...
  window._siparisUnsub = _safeListener(
    'siparis',
    // ✅ Rev 7.4: limit(300) eklendi — proposals/sales'ta yapılmıştı, siparis
    // gözden kaçmıştı. Her yeniden bağlanmada tüm koleksiyonun okunmasını sınırlar.
    () => query(_colSiparis(), orderBy('ts', 'desc'), limit(300)),
    snap => {
  ...
  // ── Analytics ─────────────────────────────────────────────────
  // ✅ Rev 7.4 — KRİTİK: Bu koleksiyon hiçbir limit/filtre olmadan
  // dinleniyordu ve bu sadece admin'e değil, HER personel cihazına da
  // uygulanıyordu. checkChanges() personelin sadece KENDİ kaydına
  // (email eşleşmesine) bakıyor, yani personel cihazları diğer TÜM
  // çalışanların TÜM geçmiş kayıtlarını gereksiz yere indiriyordu —
  // koleksiyon her gün büyüdüğü için her reconnect'te maliyet de büyüyor.
  // Artık: admin tüm koleksiyonu (raporlama için gerekli), personel
  // sadece kendi email'iyle eşleşen kayıtları dinliyor.
