...
function parseCampaigns(aciklama) {
  ...
  // ⎇ = birleşen (birlesen), 🔒︎ = kilitli
  // Strateji: her ⎇ ve 🔒︎ sembolu bir ayraç — önce konumları bul
  const ayraclar = [];
  const str = aciklama;

  // Tüm ⎇ ve 🔒︎ konumlarını bul
  for (let i = 0; i < str.length; i++) {
    if (str[i] === '⎇') {
      ayraclar.push({ idx: i, len: 1, tip: 'birlesen' });
    } else if (str.slice(i, i + '🔒︎'.length) === '🔒︎') {
      ayraclar.push({ idx: i, len: '🔒︎'.length, tip: 'kilitli' });
    }
  }

  if (ayraclar.length === 0) {
    // Ayraç yok — tüm metin tek segment
    const metin = str.trim();
    if (metin) result.push(_buildCampObj(metin, 'birlesen'));
    return result;
  }

  // Her ayraçtan önceki metni al + ayracın tipini ata
  let onceki = 0;
  ayraclar.forEach(a => {
  ...
  // Son ayraçtan sonraki kalan metin
...
async function finalizeAksiyon() {
  ...
  // Pazarlık indirimi — DOM'dan oku (nakit seçilince abakusSelection=null olur!)
  // abakusSelection varsa oradan, yoksa doğrudan input'tan al
  const _ekIndirimF    = abakusSelection?.ekIndirim
    ?? (parseFloat(document.getElementById('ab-ek-indirim')?.value) || 0);
